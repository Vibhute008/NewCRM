...
import { Plus, MoreHorizontal, CheckSquare, Square, Paperclip, Clock, CheckCircle, Trash2, Upload, ArrowRight, Edit, X, Search, Filter, RotateCcw, ArrowLeft, Code, FileText } from 'lucide-react';
...
const ProjectCard: React.FC<{ project: Project; onEdit: (p: Project) => void }> = ({ project, onEdit }) => {
  ...
  const editMilestone = (mId: string, oldTitle: string) => {
    const newTitle = prompt("Edit milestone title:", oldTitle);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files.length > 0) {
          ...
          fileList.forEach((f: any) => {
              storeFile(f.name, f); // Use name as ID for simplicity in this list, or generate UUID
  ...
  const handleOpenFile = (docName: string) => {
      const url = fileMap[docName];
      if (url) {
          window.open(url, '_blank');
      } else {
          alert(`Simulation: Opening "${docName}" from server.`);
      }
  };
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-4 hover:shadow-md transition-shadow group flex flex-col h-auto max-h-[800px]">
      ...
      {/* Progress Bar */}
      <div className="w-full bg-gray-100 rounded-full h-1.5 mb-3 shrink-0">
      ...
      {/* Milestones List - Scrollable Area */}
...
const TechLeadPanel = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-2rem)] overflow-hidden">
        ...
        <div className="flex flex-wrap gap-3 items-center w-full md:w-auto">
            ...
            <div className="relative">
                ...
                <select 
                    ...
                    className="pl-10 pr-8 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500 appearance-none bg-white cursor-pointer"
                >
      ...
      {/* Kanban Board - Flex Grow / Scrollable Content - Increased Height */}
